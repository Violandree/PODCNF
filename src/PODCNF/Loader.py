...
def make_loader(ds, batch_size, shuffle=True, drop_last=True):
...
def seed_worker(worker_id):
    # Seed for all the workers
    worker_seed = torch.initial_seed() % 2**32
    np.random.seed(worker_seed)
    random.seed(worker_seed)

def LoadData(mu, c, n_train, n_val, BATCH_SIZE, norm_scaler=None, shuffle=True, drop_last=False):
    """
